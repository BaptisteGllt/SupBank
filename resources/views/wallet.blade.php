...
@section('content')
        ...
        <section class="wallets">
            ...
            <div class="wallets__list">
                @foreach ($wallets as $wallet)
                    <a href="{{ route('showDetailWallet', [$wallet->num_wallet]) }}" class="wallet" style="background-color: #158EBD;">
                         <span class="wallet__header">
								...
								<span class="wallet__course">
									<span>{{strlen($wallet->solde)>=11 ? substr(($wallet->solde),0,9).".." : $wallet->solde}} SC</span>
									<span> {{strlen($wallet->solde *36)>=11 ? substr(($wallet->solde *36),0,9).".." : $wallet->solde *36}} €</span>
								</span>
							</span>

                        <span class="wallet__footer">
								<span>1 SC - 36 € </span>
								<span>{{$wallet->lastTransaction}}€</span>
                    </span>
    ...
    <div id="popup-new-wallet" class="mfp-container mfp-inline-holder d-none"><div class="mfp-content">
            <form class="form-prevent-multiple-submits popup popup-add-wallet zoom-anim-dialog" enctype="multipart/form-data" action="{{ action('WalletController@createNewWallet') }}" method="POST" id="add-wallet">
